...
const AppOrderResponseSchema = z.object({
	app_order: z.array(z.string()).describe("앱 ID 순서 배열 (예: ['com.webos.app.browser', 'com.webos.app.settings'])"),
});
...
appsRegistry.registerPath({
	...
	description: "사용자가 설정한 앱 ID 순서를 저장합니다. 앱 ID는 webOS 앱 식별자 (예: 'com.webos.app.browser')입니다.",
	security: [{ bearerAuth: [] }],
	request: {
		body: {
			content: {
				"application/json": {
					schema: z.object({
						order: z
							.array(z.string())
							.describe("앱 ID 순서 배열 (예: ['com.webos.app.browser', 'com.webos.app.settings'])"),
					}),
				},
			},
		},
