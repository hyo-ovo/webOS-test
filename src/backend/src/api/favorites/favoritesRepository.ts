...
class FavoritesRepository {
  async getFavoritesByUserId(userId: number) {
    const result = await query(
      "SELECT app_id FROM favorites WHERE user_id = $1 ORDER BY created_at",
      [userId]
    );
    return result.rows;
  }
  ...
  async addFavorite(userId: number, appId: string) {
    try {
      const result = await query(
        "INSERT INTO favorites (user_id, app_id) VALUES ($1, $2) RETURNING *",
        [userId, appId]
      );
  ...
  async removeFavorite(userId: number, appId: string) {
    const result = await query(
      "DELETE FROM favorites WHERE user_id = $1 AND app_id = $2 RETURNING id",
      [userId, appId]
    );
